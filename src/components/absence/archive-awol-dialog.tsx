...
import { parseArchiveAwolFormData } from "@/lib/absence/schema";

const initialState: AbsenceActionState = {};

export function ArchiveAwolDialog({
  absenceId,
  staffName,
  eventName,
  expectedUpdatedAt,
  ...
  const action = useMemo(
    () => withClientValidation(parseArchiveAwolFormData, archiveAwolAction),
    [],
  );
      ...
      >
        <div className="p-5">
          <h2 id={titleId} className="text-lg font-semibold text-slate-900">
            Archive AWOL for {staffName} at {eventName}?
          </h2>
          <p id={descriptionId} className="mt-2 text-sm text-slate-600">
            This record will leave active Staff history and the active AWOL
            Ledger but remain retained for audit.
          </p>
          <form action={formAction} noValidate className="mt-4 space-y-3">
            ...
            <label className="flex items-start gap-2 text-sm text-slate-800">
              ...
              <span>
                I understand this will archive the AWOL for {staffName} at{" "}
                {eventName}.
              </span>
            </label>
            <FieldError
            ...
            <div className="flex flex-wrap justify-end gap-2">
              ...
              >
                {pending ? "Archiving…" : "Archive AWOL"}
              </Button>
            </div>
          </form>
        </div>
