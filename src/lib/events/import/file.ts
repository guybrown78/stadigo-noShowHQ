...
export function validateImportFileBytes(
  ...
  if (bytes.byteLength === 0) {
    return "The file is empty. Download the template, add your events, and try again.";
  }
  if (bytes.byteLength > MAX_IMPORT_FILE_BYTES) {
    return `The file is larger than ${MAX_IMPORT_FILE_BYTES / (1024 * 1024)} MB. Split the programme into smaller files and try again.`;
  }
...
export function headerIndexMap(
  headers: string[],
): { ok: true; index: Record<EventImportHeader, number> } | { ok: false; error: string } {
  const index = {} as Record<EventImportHeader, number>;
  ...
  const missing = EVENT_IMPORT_HEADERS.filter((header) => !seen.has(header));
  if (missing.length > 0) {
    return {
      ok: false,
