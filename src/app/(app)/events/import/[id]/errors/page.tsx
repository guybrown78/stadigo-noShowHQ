...
import { importErrorsHref } from "@/lib/events/url";
...
export default async function ImportErrorsPage({
  ...
  if (record.status !== EventImportStatus.VALIDATION_FAILED) {
  ...
  return (
      <div>
        ...
        <p className="mt-2 max-w-2xl text-slate-600">
          No venues or events were created. Fix the rows below, then upload the
          corrected file. Valid rows are not imported on their own.
        </p>
        ...
        <dl className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <SummaryCard label="Rows read" value={record.totalRows} />
          <SummaryCard label="Valid rows" value={record.validRows} />
          <SummaryCard label="Rows with errors" value={record.invalidRows} />
          <SummaryCard label="Matched existing venues" value={record.matchedVenueCount} />
          <SummaryCard label="New venues detected" value={record.newVenueCount} />
          <SummaryCard label="Potential duplicate events" value={record.duplicateEventCount} />
        </dl>
        ...
        >
          <label htmlFor="import-error-q" className="mb-1 block text-sm font-medium text-slate-700">
            Search errors
          </label>
            ...
            <tbody>
              {list.rows.flatMap((row) => {
                ...
                return lines.map(([field, messages]) => (
                  <tr key={`${row.id}-${field}`} className="border-b border-slate-100 align-top">
                    <td className="whitespace-nowrap px-4 py-3 text-slate-700">
                      {row.sourceRowNumber}
                    </td>
                    <td className="px-4 py-3 text-slate-700">{importFieldLabel(field)}</td>
                    <td className="px-4 py-3 text-slate-600">
                      {displaySubmittedValue(rawRow, field)}
                    </td>
                    <td className="px-4 py-3 text-slate-800">{messages.join(" ")}</td>
                  </tr>
                ));
              })}
            </tbody>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          <a
            href={`/events/import/${id}/errors/download`}
...
function displaySubmittedValue(
  ...
  const map: Record<string, keyof typeof raw> = {
    reference: "Reference",
    name: "Event Name",
